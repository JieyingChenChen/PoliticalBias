...
def get_dataset(DEBUG, small_data_size = 20, variant=0, exp="ideology", lang="NO", replace_start=0, drop_motiontypes=False):
    cols = []


    # For LLMs
    if variant == 0:
        cols = ['id', 'sak_id', 'votering_id', 'forslag_tekst' if replace_start !=2 else 'forslag_tekst_modified', 'vedtatt', 'number', 'dokumentgruppe', 'category']
    # Categories
    elif variant == 1:
        cols = ['id', 'sak_id', 'behandlet_sesjon_id', 'forslag_tekst' if replace_start !=2 else 'forslag_tekst_modified', 'category', 'dokumentgruppe']
    # parties votes
    elif variant == 2:
        cols = ['id', 'behandlet_sesjon_id', 'forslag_partier', 'category', 'dokumentgruppe', 'forslag_tekst' if replace_start !=2 else 'forslag_tekst_modified']  
        cols = cols + [f"{p}_vote" for p in party_codes]  # e.g. ['partyA_result', 'partyB_result', ...]
        print(cols)
    elif variant == 3:
        cols = None
    else:
    ...
    fname = "" 
    if exp == "category": # for annotating with missing cat
        fname =f"data/all_votes_no_cat.csv"
    else:
        fname = f"data/all_motions_2018-2023_pros.csv"
    ...
    # drop motion types
    if drop_motiontypes:
        keep = ['representantforslag', 'proposisjon', 'redegjorelse']
        print("Print before drop motion types:", len(df))
        df = df[df['dokumentgruppe'].isin(keep)]
        print("Print after drop motion types:", len(df))
    
    if replace_start == 2:
        # drop NA / empty
        print("before drop NA imperative", len(df))
        df = df[df['forslag_tekst_modified'].notna() & (df['forslag_tekst_modified'] != "")].copy()
        df.rename(columns={'forslag_tekst_modified': 'forslag_tekst'}, inplace=True)
        print("after drop NA imperative",len(df))

    # Do for all to ensure same no of samples
    #if variant == 1:
    norwegian_to_english = {komite_dict_NO[key]: komite_dict[key] for key in komite_dict_NO if key in komite_dict}
    
    df_pred_cat = pd.read_csv("results/gpt-4o-mini_results_category_NOR_prompt=1,REM=1.csv")
    ...
    df.loc[mask, 'category'] = df.loc[mask, 'gpt-4o-mini_category'].map(norwegian_to_english).fillna(df.loc[mask, 'gpt-4o-mini_category'])
    ...
    if 'category' in df.columns:
        df['category'] = df['category'].replace('Utdanning og forskning', 'Education and research')
    ...
    #if variant == 2:
    #   party_codes_set = set(party_codes)
    #    df['forslag_partier'] = df['forslag_partier'].apply(lambda lst: [party for party in lst if party in party_codes_set])
    #elif variant == 2:
    #    df.drop(['votering_id', 'number', 'forslag_tekst'], axis=1, inplace=True)

    if lang == "EN":
        if replace_start != 2:
            fname = "results/gpt-4o-mini_results_translate_NOR_prompt=1.csv"
            df2 = pd.read_csv(fname)
        else:
            fname = "results/gpt-4o-mini_results_translate_NOR_prompt=1,REM=2.csv"
            df2 = pd.read_csv(fname)
            
        #df2.dropna(subset=['gpt-4o-mini_vote'], inplace=True)
        df = df.merge(df2, on='id', how='inner')  # Change 'inner' to 'left', 'right', or 'outer' as needed
        print(df.columns)
        df.rename(columns={"gpt-4o-minitranslate": "gpt-4o-mini_translate"}, inplace=True, errors="ignore")
        print(df.columns)
        df['forslag_tekst'] = df['gpt-4o-mini_translate']
        df.drop(['gpt-4o-mini_translate','Unnamed: 0'], axis=1, inplace=True, errors='ignore')
    
    if DEBUG:
        df = df.sample(n=small_data_size, random_state=RANDOM_SEED)
        print(df['id'].tolist())
    return df
...
def plot_landscape(df, title, models, exp, x_pts=None, y_pts=None):
    ...
    if exp == "NL":
        columns = ['PVV', 'GL-PvdA', 'VVD', 'NSC', 'D66', 'BBB', 'CDA', 'SP', 'ChristenUnie', 'DENK', 'PvdD', 'SGP', 'FVD', 'Volt', 'JA21'] + models
         ...
         columns = ['A', 'H', 'Sp', 'FrP', 'KrF', 'MDG', 'SV', 'V', 'R', 'PF'] + models
